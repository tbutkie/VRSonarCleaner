...
void SelectAreaBehavior::update()
{
	...
	if (castRay(rayOrigin, rayDirection, planeOrigin, planeNormal, &m_vec3CurrentLocationOnPlane) && m_pDataVolumeSelection->isWorldCoordPointInBounds(m_vec3CurrentLocationOnPlane, false))
		m_bShowCursor = true;
	else
		m_bShowCursor = false;
	...
	if (m_bSelectingArea)
	{
		...
		m_dvec3MinBound.z = std::min(tmpStart.z, tmpNow.z);

		m_dvec3MaxBound.x = std::max(tmpStart.x, tmpNow.x);
		m_dvec3MaxBound.y = std::max(tmpStart.y, tmpNow.y);
		m_dvec3MaxBound.z = std::max(tmpStart.z, tmpNow.z);
...
void SelectAreaBehavior::draw()
{
	if (m_bSelectingArea)
	{
		...
		Renderer::getInstance().drawPrimitive("icosphere", trans, glm::vec4(1.f, 0.f, 0.f, 0.5f), glm::vec4(1.f, 1.f, 1.f, 0.5f), 10.f);
	}
