...
{		
	...
	m_vec3ScreenNormal = glm::normalize(glm::vec3(0.f, -0.5f, 1.f));
	...
	sviMono->view = glm::lookAt(cam->pos, cam->lookat, cam->up);
...
void FishTankScene::update()
{
	...
	if (m_pTDM->getPrimaryController() && m_pTDM->getPrimaryController()->justPressedTouchpad())
	{
		...
		cam->pos = m_vec3ScreenCenter + m_vec3ScreenNormal * 0.57f - glm::normalize(glm::cross(m_vec3ScreenNormal, m_vec3ScreenUp)) * 0.1f;
		cam->lookat = cam->pos - glm::dot(cam->pos - m_vec3ScreenCenter, m_vec3ScreenNormal) * m_vec3ScreenNormal;
	}
...
{
	Renderer::getInstance().drawPrimitive("cube", glm::translate(glm::mat4(), m_vec3ScreenCenter - m_vec3ScreenNormal * 0.05f) * glm::scale(glm::mat4(), glm::vec3(0.1f)), glm::vec4(1.f, 0.f, 0.f, 1.f));
