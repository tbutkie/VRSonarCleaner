...
HairyVolume::HairyVolume(CosmoVolume* cosmoVolume)
...
HairyVolume::~HairyVolume()
{
	destroyGeometry();
}

void HairyVolume::update()
{
	...
	if (m_bOscillate)
	{
		m_qVolumeOrientation = glm::rotate(glm::mat4(), glm::radians(rotNow), glm::vec3(0.f, 1.f, 0.f));
	}
	else
	{
...
std::string HairyVolume::getShaderName()
{
	return m_vstrShaderNames[m_iCurrentShader];
}

bool HairyVolume::setShader(std::string shaderName)
{
...
void HairyVolume::nextGeomStyle()
{
...
void HairyVolume::prevGeomStyle()
{
...
std::string HairyVolume::getGeomStyle()
{
	return m_vstrGeomStyleNames[m_iCurrentGeomStyle];
}

bool HairyVolume::setGeomStyle(std::string geomType)
{
...
void HairyVolume::reseed()
{
	m_vvec3StreamlineSeeds.clear();

	glm::vec3 xDir(1.f, 0.f, 0.f);
	glm::vec3 yDir(0.f, 1.f, 0.f);
	glm::vec3 zDir(0.f, 0.f, 1.f);

	float xCellSize = m_pCosmoVolume->getDimensions().x / static_cast<float>(m_uiSamplingResolution);
	float yCellSize = m_pCosmoVolume->getDimensions().y / static_cast<float>(m_uiSamplingResolution);
	float zCellSize = m_pCosmoVolume->getDimensions().z / static_cast<float>(m_uiSamplingResolution);
	
	float maxJitterX = 0.25f * xCellSize;
	float maxJitterY = 0.25f * yCellSize;
	float maxJitterZ = 0.25f * zCellSize;

	for (int i = 0; i < m_uiSamplingResolution; ++i)
	{
		float ratioWidth = m_uiSamplingResolution == 1 ? 0.f : (float)i / (m_uiSamplingResolution - 1) - 0.5f;

		for (int j = 0; j < m_uiSamplingResolution; ++j)
		{
			float ratioHeight = m_uiSamplingResolution == 1 ? 0.f : (float)j / (m_uiSamplingResolution - 1) - 0.5f;

			for (int k = 0; k < m_uiSamplingResolution; ++k)
			{
				float ratioDepth = m_uiSamplingResolution == 1 ? 0.f : (float)k / (m_uiSamplingResolution - 1) - 0.5f;

				glm::vec3 pos = xDir * ratioWidth * static_cast<float>(m_pCosmoVolume->getDimensions().x - xCellSize) - yDir * ratioHeight * static_cast<float>(m_pCosmoVolume->getDimensions().y - yCellSize) - zDir * ratioDepth * static_cast<float>(m_pCosmoVolume->getDimensions().z - zCellSize);

				if (m_bJitterSeeds)
					pos += m_Distribution(m_RNG) * xDir * maxJitterX + m_Distribution(m_RNG) * yDir * maxJitterY + m_Distribution(m_RNG) * zDir * maxJitterZ;
...
void HairyVolume::sampleVolume()
{
	...
	for (auto &seed : m_vvec3StreamlineSeeds)
	{
		glm::dvec3 domainSeed = m_pCosmoVolume->convertToRawDomainCoords(glm::vec4(seed, 1.f));
...
void HairyVolume::rebuildGeometry()
{
	destroyGeometry();
...
void HairyVolume::buildStreamLines()
{
	struct PrimVert {
		glm::vec3 p; // point
		glm::vec4 c; // color
...
void HairyVolume::buildStreamTubes(float radius)
{
	...
	{
		glCreateBuffers(1, &m_glVBO);
		glNamedBufferStorage(m_glVBO, m_uiSamplingResolution * m_uiSamplingResolution * m_uiSamplingResolution * (103 * numCircleVerts + 2) * sizeof(PrimVert), NULL, GL_DYNAMIC_STORAGE_BIT);
	}

	//if (!m_glEBO)
	{
		glCreateBuffers(1, &m_glEBO);
		glNamedBufferStorage(m_glEBO, m_uiSamplingResolution * m_uiSamplingResolution * m_uiSamplingResolution * (100 * 6 + 2 * 3) * m_uiNumTubeSegments * sizeof(GLuint), NULL, GL_DYNAMIC_STORAGE_BIT);
	}
...
void HairyVolume::buildStreamCones(float radius)
{
	glm::vec3 dimratio =  m_pCosmoVolume->getOriginalDimensions() / m_pCosmoVolume->getDimensions();
	// For each streamline segment
	// construct local coordinate frame matrix using orthogonal axes u, v, w; where the w axis is the segment (Point_[n+1] - Point_n) itself
...
void HairyVolume::destroyGeometry()
{
	for (auto &buff : { m_glVBO, m_glEBO })
		if (buff)
...
void HairyVolume::initParticles()
{
...
void HairyVolume::updateParticles(float elapsedTime)
{
...
void HairyVolume::drawParticleHeads(float radius)
{
	for (int i = 0; i < m_nParticleCount; ++i)
	{
...
void HairyVolume::buildParticleTails()
{
	struct PrimVert {
		glm::vec3 pos;
		glm::vec4 color;
...
glm::quat HairyVolume::getSegmentOrientationMatrixNormalized(glm::vec3 segmentDirection, glm::vec3 up)
{
